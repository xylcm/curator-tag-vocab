...
| 状态 | 当前值 | 新值 |
|------|--------|------|
| 背景色（默认） | `#e3f2ff`（浅蓝） | `#fff8e1`（淡黄） |
| 文字色 | `#007aff`（蓝色） | `#f9a825`（琥珀色） |
| 边框色（默认） | `#b8dcff`（浅蓝） | `#ffecb3`（浅黄） |
| 背景色（hover） | `#d0e8ff`（浅蓝） | `#ffecb3`（浅黄） |
| 边框色（hover） | `#9fceff`（浅蓝） | `#ffe082`（黄色） |
...
- **对比度测试**: 确认文字颜色 `#f9a825` 在背景 `#fff8e1` 上可读性良好
