---
name: implement-prd
description: 基于已评审通过的 PRD 文档，理解项目架构，完成需求的代码实现并更新 PR。Use when implementing a feature from an approved PRD, or when the user says "实现PRD", "implement PRD", or "develop feature".
argument-hint: "[issue-number] [pr-number]"
disable-model-invocation: true
allowed-tools: Bash, Read, Write, Edit, MultiEdit, Glob, Grep
---

# PRD Feature Development Instructions

## Arguments

- `$0` — GitHub Issue 编号
- `$1` — Pull Request 编号（该 PR 在 write-prd 阶段已创建）
...
1. **清理代码**：撤销所有未提交的变更，确保不留下不完整的实现
   ...
   ```
2. **标记失败**：`gh pr edit $1 --add-label "status:failed"`
3. **说明原因**：在 PR #$1 中添加评论，清晰说明失败的具体原因
...
拉取并切换到 `agent/issue-$0` 分支：

```bash
git fetch origin agent/issue-$0
git checkout agent/issue-$0
git pull origin agent/issue-$0
```
...
更新已有 PR #$1（该 PR 在 PRD 阶段已创建，不要新建）：
- 标题：简短、清晰地说明本次需求内容
  - 功能开发以 `feat:` 开头
  - Bug 修复以 `bugfix:` 开头
- 正文模板：
...
在 PR #$1 中添加评论告知需求开发完成，并添加标签 `status:code-review`。
